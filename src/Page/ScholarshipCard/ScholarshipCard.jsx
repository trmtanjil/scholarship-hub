import { useQuery } from '@tanstack/react-query';
import { Link } from 'react-router';
import useAxiosSecure from '../../hoocks/useAxiosSecure';

const ScholarshipCard = ({ scholarship }) => {
  const {
    _id,
    scholarshipName,
    universityName,
    subjectCategory,
    scholarshipCategory,
    applicationFees,
    postDate,
    universityImage,
  } = scholarship;

  const axiosSecure = useAxiosSecure();

  // 🔁 Get average rating from reviews collection
  const { data: ratingData, isLoading } = useQuery({
    queryKey: ['averageRating', _id],
    queryFn: async () => {
      const res = await axiosSecure.get(`/reviews/average-rating/${_id}`);
      return res.data;
    },
  });

  return (
    <div className="card bg-white rounded-lg shadow-md p-4">
      <img
        src={universityImage}
        alt={scholarshipName}
        className="rounded-lg h-48 w-full object-cover"
      />
      <h3 className="text-lg font-semibold mt-2">{scholarshipName}</h3>
      <p className="text-sm text-gray-600">University: {universityName}</p>
      <p className="text-sm text-gray-600">Subject: {subjectCategory}</p>
      <p className="text-sm text-gray-600">Category: {scholarshipCategory}</p>
      <p className="text-sm text-gray-600">Application Fee: ${applicationFees}</p>
      <p className="text-xs text-gray-500">Posted: {postDate}</p>

      {/* ⭐️ Show average rating */}
      {isLoading ? (
        <p className="text-sm text-gray-400 italic mt-1">Loading rating...</p>
      ) : (
        <p className="text-sm text-yellow-600 font-semibold mt-1">
          ⭐ {ratingData?.averageRating || 0} / 5
        </p>
      )}

      <Link to={`/sholarshipdetails/${_id}`}>
        <button className="btn btn-sm mt-3 w-full bg-blue-600 text-white">
          Details
        </button>
      </Link>
    </div>
  );
};

export default ScholarshipCard;
