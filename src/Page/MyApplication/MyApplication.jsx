...
const MyApplication = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/applied-scholarships?email=${user?.email}`
      );
      return res.data;
    },
    enabled: !!user?.email,
  ...
  const handleReviewSubmit = async (e) => {
    ...
    const reviewData = {
      ...
      scholarshipName:
        selectedScholarship?.scholarshipName || selectedScholarship?.subject,
    ...
    <div className="p-4">
                  ...
                  <button
                    onClick={() => {
                      if (app.status === "pending") {
                        ...
                        Swal.fire(
                          "⚠️ Cannot Edit",
                          "Your application is already processing or completed.",
                          "warning"
                        );
                      }
                    }}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white text-white rounded-lg p-6 w-96 space-y-4">
