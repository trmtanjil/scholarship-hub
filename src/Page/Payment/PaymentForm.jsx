...
function PaymentForm() {
  ...
  // Check if current user already paid
  useEffect(() => {
    if (user?.email && scholarId) {
      axiosSecure
        .get(`/payments/check?email=${user.email}&scholarId=${scholarId}`)
        .then((res) => {
          setAlreadyPaid(res.data.alreadyPaid);
        });
    }
  }, [user, scholarId, axiosSecure]);

  const amount = parcelInfo.applicationFees;
  const amountInCents = amount * 100;
  const isAlreadyPaid = alreadyPaid || paymentSuccess;
  ...
  return (
    <div>
      ...
      >
        ...
        >
          {isAlreadyPaid ? "Already Paid" : `Pay ৳${amount}`}
        </button>
